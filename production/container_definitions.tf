data "template_file" "app_task_definition" {
  template = <<DEFINITION
    [
      {
        "name": "nginx",
        "image": "${data.terraform_remote_state.common.outputs.nginx_repository_url}:production",
        "cpu": ${var.fargate_cpu},
      ...
      {
        "name": "app",
        "image": "${data.terraform_remote_state.common.outputs.production_app_repository_url}:latest",
        "cpu": ${var.fargate_cpu},
        "memory": ${var.fargate_memory},
        "essential": true,
        "networkMode": "awsvpc",
